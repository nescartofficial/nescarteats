...
class Messages
{
	...
	{
		$message = "<p style='font-size: 1.1rem;'>Were sending this email to confirm that your account at Nescarteats was created successfully.</p>
	...
	public static function verifyEmail($token, $name, $email = null)
	{
		...
		$message = "<p style='font-size: 1.1rem;'>You registered an account on Nescarteats, before being able to make a purchase you need to verify that this is your email address by clicking the link or the button below.<br><br> {$link}</p><br><br>
	...
	public static function verifySellerEmail($token, $name, $email = null)
	{
		...
		$message = "<p style='font-size: 1.1rem;'>Your Seller Center account has been successfully created.</p>
                    <p style='font-size: 1.1rem;'>You registered as a seller on Nescarteats Seller Center with this email address. </p>
	...
	{
		$message = "<p style='font-size: 1.1rem;'>Thank you for visiting Nescarteats and placing your first order! </p>
	...
	public static function sendSMS($mobiles = "2348030000000", $message = "This is a test message", $sender = "Nescart Eats")
	{
	    
        $curl = curl_init();
        $data = array("api_key" => "TLTQAT8Iqt6cRN6fuW0YC9dW4j47ut6RFYuEmQK6OEkm2OJ139GI9XoPIVKpgC", "to" => $mobiles,  "from" => "Nescart Eats",
        "sms" => strip_tags($message),  "type" => "plain",  "channel" => "generic" );
		...
		if ($result && isset($result->message) && $result->message == "Successfully Sent") {
