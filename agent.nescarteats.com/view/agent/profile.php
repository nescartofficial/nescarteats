...
$world = new World();
$Agents = new General('profiles');
$sellers = new General('sellers');

$us = $user->data();
$agent = $User->getAgent($user->data()->id);
...
<div class="row">
    <div class="col-md-12 mx-auto">
        <form action="controllers/profile.php" name="profile_form" id="profile_form" method="post" enctype="multipart/form-data">
            <input type="hidden" name="rq" value="<?= $agent ? 'edit' : 'setup'; ?>">
                    ...
                    <div class="row">
                        ...
                        <div class="col-md-6 mb-4">
                            <div class="d-flex align-items-end gap-5">
                                <div>
                                    <label for="cover" class="d-block"><?= $agent ? 'Change' : null ?> Profile Picture <i class="text-danger">*</i></label>
                                    <input type="file" name="cover" id="cover" class="form-control">
                                    <span class="text-help">Upload a photo of yourself. Picture size is (300x300) pixels. </span>
                                </div>
                                <?php if ($agent) { ?>
                                    <img src='media/images/profile/<?= $agent->image ?>' alt='' class='img-fluid' style='height: 62px; width: 62px; object-fit:cover;'>
                                <?php } ?>
                            </div>
                        </div>
                    </div>
                ...
                <div class="card-body">
                    ...
                    <div class="row">
                            ...
                            <select name="country" id="country" data-type="country" data-world-target="#seller-state" data-placeholder="Select country" class="world sa-select2 form-control form-control-lg form-select">
                                <?php if ($countries) { ?>
                                    ...
                                    <?php foreach ($countries as $k => $v) { ?>
                                        <option value="<?= $v->id ?>" <?= $agent && $agent->country == $v->id ? 'selected'  : null; ?>><?= $v->name ?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 mb-4">
                            ...
                            <select name="state" id="seller-state" data-type="state" data-world-target="#seller-city" data-placeholder="Select State" data-selected="<?= $agent ? $agent->state : null; ?>" class="world sa-select2 form-control form-control-lg form-select">
                                <option value="">Select a country</option>
                                <?php if ($agent && $agent->country) {
                                    $states = $world->getStatesByCountryId($agent->country);
                                ?>
                                    <?php foreach ($states as $k => $v) { ?>
                                        <option value="<?= $v->id ?>" <?= $agent && $agent->state == $v->id ? 'selected'  : null; ?>><?= $v->name ?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-4 mb-4">
                            ...
                            <select name="city" id="seller-city" data-placeholder="Select City" data-selected="<?= $agent ? $agent->city : null; ?>" class="city sa-select2 form-control form-control-lg form-select">
                                <option value="">Select a state</option>
                                <?php if ($agent && $agent->state) {
                                    $cities = $world->getCitiesByStateId($agent->state);
                                ?>
                                    <?php foreach ($cities as $k => $v) { ?>
                                        <option value="<?= $v->id ?>" <?= $agent && $agent->city == $v->id ? 'selected'  : null; ?>><?= $v->name ?></option>
                                    <?php } ?>
                                <?php } ?>
                            </select>
                        </div>
                        ...
                        <div class="col-md-12 mb-4">
                            ...
                            <input type="text" name="address" value="<?= $agent ? $agent->address : ($form_data ? $form_data['address'] : null); ?>" id="address" class="form-control form-control-lg" placeholder="Input address" required>
                        </div>
                    </div>
                </div>
            </div>
