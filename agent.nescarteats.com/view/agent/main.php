...
if ($agent) {
    ...
    $order_details_list = $Orders->getVendorOrders($agent->id);
    // print_r($Orders->getVendorOrdersDistinct($agent->id, null, 'order_id', " AND status = 'pending' "));
    // Others
    ...
    $searchTerm = "WHERE id > 0 AND user_id = {$agent->user_id} ";
    $recent_products = $Menus->getPages(4, 0, $searchTerm);

    // Recent Orders
    $recent_orders = $Orders->getVendorOrders($agent->id, null, null, "ORDER BY created_at DESC", 4, 0);
    // Pending Orders
    $pending_orders = $Orders->getVendorOrders($agent->id, null, " AND status = 'pending' ", "ORDER BY created_at DESC", 4, 0);
    // Awaiting Delivery
    $awaiting_orders = $Orders->getVendorOrders($agent->id, null, " AND status = 'pending' ", "ORDER BY created_at DESC", 4, 0);
        ...
        <?php if (Input::get('action')) {
            ...
            <?php if (!$agent->is_verified) { ?>
                <div class="alert alert-danger text-center mb-4">
                    <span>We're reviewing your account! Please expect feedback within 24 hours via your email address provided during registration. Thank You.</span>
                </div>
            <?php } ?>
                                ...
                                <div class="card-body">
                                    ...
                                    <?php if ($recent_orders) { ?>
                                        <?php foreach ($recent_orders as $order) {
                                            $details = $Orders->getDetails($order->order_id, " AND vendor_id = {$agent->id} ");
