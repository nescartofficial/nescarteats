...
$backto = Input::get('backto') ? '../' . Input::get('backto') : '../sms-snippets';
if (
    $user->isAdmin() &&
    Input::exists('get') &&
    Input::get('rq') &&
    ...
    switch (trim(Input::get('rq'))) {
        case 'status':
            $found = Input::get('id') ? $sms_snippets->get(Input::get('id')) : null;
            if ($found) {
                $sms_snippets->update(array('status' => !$found->status), $found->id);
