...
$ads = new General('ads');
$ads_positions = new General('ads_positions');

$items = $ads->getAll();
$position_list = $ads_positions->getAll();
$item = Input::get('action') && Input::get('action') == 'edit' && Input::get('sub') && is_numeric(Input::get('sub')) ? $ads->get(Input::get('sub')) : null;
                ...
                <div class="py-5">
                    <div class="row g-4 align-items-center">
                        <div class="col">
                            <nav class="mb-2" aria-label="breadcrumb">
                                <ol class="breadcrumb breadcrumb-sa-simple">
                                    ...
                                    <li class="breadcrumb-item"><a href="ads">Adverts</a></li>
                        ...
                        <div class="col-auto d-flex">
                            <a href="#" class="btn btn-secondary me-3">Duplicate</a><button type="submit" form="page_form" class="btn btn-primary">Save</button>
                        </div>
                    </div>
                </div>
                <form action="controllers/ads.php" method="post" enctype="multipart/form-data" name="page_form" id="page_form">
                                ...
                                <div class="card">
                                    <div class="card-body p-5">
                                        ...
                                        <div class="mb-4">
                                            ...
                                            <select name="position" id="position" class="sa-select2 form-select" style="width: 100%;" required>
                                                <option value="">Select ads position</option>
                                                <?php foreach ($position_list as $k => $v) { ?>
                                                    <option value="<?= $v->id ?>" <?= $item && $item->position == $v->id ? 'selected' : null; ?>><?= $v->title; ?></option>
                                                <?php } ?>
                                            </select>
                                        </div>
                                    </div>
                                    ...
                                    <div class="card-body p-5">
                                        ...
                                        <?php if ($item) { ?>
                                            <div class="border p-4 d-flex justify-content-center">
                                                <div class="max-w-20x">
                                                    <img src="<?= SITE_URL ?>media/images/ads/<?= $item->image; ?>" class="w-100 h-auto" width="320" height="320" alt="" />
                                                </div>
                                            </div>
                                        <?php } ?>
        ...
        <div class="py-5">
            <div class="row g-4 align-items-center">
                <div class="col">
                    ...
                    <h1 class="h3 m-0">Adverts</h1>
                </div>
                <div class="col-auto d-flex"><a href="ads/add" class="btn btn-primary">New</a></div>
            </div>
        </div>
        <div class="card">
                ...
                <tbody>
                    ...
                    <?php if ($items) { ?>
                        <?php foreach ($items as $index => $v) {
                            $position = $ads_positions->get($v->position); ?>
                            <tr>
                                <td><input type="checkbox" class="form-check-input m-0 fs-exact-16 d-block" aria-label="..." /></td>
                                <td>
                                    <div class="d-flex align-items-center">
                                        <a href="ads/edit/<?= $v->id; ?>" class="me-4">
                                            <div class="sa-symbol sa-symbol--shape--rounded sa-symbol--size--lg">
                                                <img src="<?= SITE_URL ?>media/images/ads/<?= $v->image ?>" width="40" height="40" alt="" style="object-fit:cover;" />
                                            </div>
                                        </a>
                                        <div>
                                            <a href="ads/edit/<?= $v->id; ?>" class="text-reset"><?= $v->title; ?></a>
                                        </div>
                                    </div>
                                </td>
                                <td><?= $position->title; ?></td>
                                <td>
                                    <a href="controllers/ads.php?rq=status&id=<?= $v->id; ?>">
                                ...
                                <td>
                                    <div class="dropdown"><button class="btn btn-sa-muted btn-sm" type="button" id="category-context-menu-0" data-bs-toggle="dropdown" aria-expanded="false" aria-label="More"><svg xmlns="http://www.w3.org/2000/svg" width="3" height="13" fill="currentColor">
                                        ...
                                        <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="category-context-menu-0">
                                            <li><a class="dropdown-item" href="ads/edit/<?= $v->id; ?>">Edit</a></li>
                                            <li>
                                                <hr class="dropdown-divider" />
                                            </li>
                                            <li><a class="dropdown-item text-danger" onclick="return confirm('Are you sure to delete this item?');" href="controllers/ads.php?rq=delete&id=<?= $v->id; ?>">Delete</a></li>
                                        </ul>
                                    </div>
                                </td>
                            </tr>
