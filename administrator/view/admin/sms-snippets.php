...
$items = $sms_snippets->getAll();

$item = Input::get('action') && Input::get('action') == 'edit' && Input::get('sub') && is_numeric(Input::get('sub')) ? $sms_snippets->get(Input::get('sub')) : null;
                ...
                <div class="py-5">
                    <div class="row g-4 align-items-center">
                        <div class="col">
                            ...
                            <h1 class="h3 m-0"><?= $item ? 'Edit' : 'Add' ?> SMS Snippet</h1>
                        </div>
                        <div class="col-auto d-flex"><button type="submit" form="testimonial" class="btn btn-primary">Save</button></div>
                    </div>
                </div>
                <form action="controllers/sms.php" method="post" enctype="multipart/form-data" name="testimonial" id="testimonial">
            ...
            <div class="py-5">
                <div class="row g-4 align-items-center">
                    <div class="col">
                        ...
                        <h1 class="h3 m-0">SMS Snippets</h1>
                    </div>
                    <div class="col-auto d-flex"><a href="sms-snippets/add" class="btn btn-primary">New Snippet</a></div>
                </div>
            </div>
            <div class="card">
                    ...
                    <tbody>
                        <?php if ($items) { ?>
                            <?php foreach ($items as $index => $v) { ?>
                                <tr>
                                    <td><a href="sms-snippets/edit/<?= $v->id; ?>" class="text-reset"><?= $v->title; ?></a></td>
                                    <td><?= $v->subject; ?></td>
                                    <td>
                                        <p><?= $v->message; ?></p>
                                    </td>
                                    <td>
                                        <a href="controllers/sms.php?rq=snippet-status&id=<?= $v->id; ?>">
                                    ...
                                    <td>
                                        <div class="dropdown"><button class="btn btn-sa-muted btn-sm" type="button" id="category-context-menu-0" data-bs-toggle="dropdown" aria-expanded="false" aria-label="More"><svg xmlns="http://www.w3.org/2000/svg" width="3" height="13" fill="currentColor">
                                            ...
                                            <ul class="dropdown-menu dropdown-menu-end" aria-labelledby="category-context-menu-0">
                                                <li><a class="dropdown-item" href="sms-snippets/edit/<?= $v->id; ?>">Edit</a></li>
                                                <li>
                                                    <hr class="dropdown-divider" />
                                                </li>
                                                <li><a class="dropdown-item text-danger" onclick="return confirm('Are you sure to delete this item?');" href="controllers/sms.php?rq=snippet-delete&id=<?= $v->id; ?>">Delete</a></li>
                                            </ul>
                                        </div>
                                    </td>
                                </tr>
