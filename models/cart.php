            ...
            if ($item) {
                if (Session::exists('cart')) {
                    ...
                    if ($in_cart) {
                        ...
                        array_push($cart, array('id' => Input::get('pid'), 'vendor' => $item->vendor_id, 'name' => $item->title, 'amount' => $item->price, 'total_amount' => $item->price, 'quantity' => 1, 'addons' => array(), 'variations' => array(), 'status' => 'pending'));
                    }
                } else {
                    array_push($cart, array('id' => Input::get('pid'), 'vendor' => $item->vendor_id, 'name' => $item->title, 'amount' => $item->price, 'total_amount' => $item->price, 'quantity' => 1, 'addons' => array(), 'variations' => array(), 'status' => 'pending'));
                }
