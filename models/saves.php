...
class Saves
{
    private $_db, $_data, $_table = 'saved_menus';

    function __construct($data = 'saved_menus')
    {
    ...
    public function getSaves($field = 'menu_id', $table2 = 'menus', $user_id = null)
    {
        $user = $user_id ? $user_id : null;
        $user = !$user && $this->_user->isLoggedIn() ? $this->_user->data()->id : $user;

        if (!$this->_db->query("
            SELECT * FROM {$table2} 
            INNER JOIN {$this->_table} 
            ON {$table2}.id = {$this->_table}.{$field}
            WHERE {$this->_table}.user_id = {$user}")->error()) {
            return $this->_db->results();
        }
        return false;
    }
