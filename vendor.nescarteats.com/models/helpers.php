...
class Helpers
{
	...
	public static function date_difference($first_date = "2012-11-30 17:03:30", $second_date = "2012-12-21 00:00:00", $format = 'minutes') {
        $fd = new DateTime($first_date);
        $sd = new DateTime($second_date);
        
        $interval = $fd->diff($sd);
    
        $result = "";
        
        if ($format ==  'years' && $interval->y) { return $interval->format("%y"); }
        if ($format ==  'months' && $interval->m) { return $interval->format("%m"); }
        if ($format ==  'days' && $interval->d) { return $interval->format("%d"); }
        if ($format ==  'hours' && $interval->h) { return $interval->format("%h"); }
        if ($format == "minutes" && $interval->i) { return $interval->format("%i"); }
        if ($format ==  'seconds' && $interval->s) { return $interval->format("%s"); }
    
        return $interval->format("%y years %m months %d day %h hours %i minutes %s seconds");
    }
