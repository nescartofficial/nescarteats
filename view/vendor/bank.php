                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                                ...
                                <select name="bank" id="bank" data-type="bank" data-placeholder="Select bank" data-world-target="#seller-state" class="world sa-select2 form-control form-control-lg form-select" required>
                                    ...
                                    <?php if ($bank_list) { ?>
                                        <?php foreach ($bank_list as $k => $v) { ?>
                                            <option value="<?= $v->id; ?>" <?= $seller_bank && $seller_bank->bank == $v->id ? 'selected' : null ?>><?= $v->name; ?></option>
                                        <?php } ?>
                                    <?php } ?>
                                </select>
                            </div>
                            ...
                            <div class="col-md-4 mb-4">
                                ...
                                <input type="text" name="account_number" value="<?= $seller_bank ? $seller_bank->account_number : ($form_data ? $form_data['account_number'] : null); ?>" id="account_number" class="form-control" placeholder="Input account number" required>
                            </div>
                            <div class="col-12"></div>
                            <div class="col-md-4 mb-4">
                                <label for="account_name" class="form-label">Bank Name</label>
                                <input type="text" name="account_name" value="<?= $seller_bank ? $seller_bank->account_name : ($form_data ? $form_data['account_name'] : null); ?>" id="account_name" class="form-control" placeholder="Input bank name" required>
                            </div>
                        </div>
                    </div>
                </div>
